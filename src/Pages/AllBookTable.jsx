...

            {
              ...
              <tr>
                ...
                <td className="font-bold flex flex-col lg:flex-row ">{book.quantity}
                  <span className="badge badge-ghost bg-orange-100 font-normal badge-md ml-2">Rating : {book.rating} </span>
                </td>
                </div>

                <td>{book.author}</td>
                <th>
                <Link to={`/updatebook/${book._id}`} className="btn btn-accent hover:bg-sky-400 hover:border-none scale-75 duration-300   hover:text-white" disabled={book.bookAdderEmail !== user.email}>Update Book</Link>
                </th>
              </tr>
            </tbody>
                )
